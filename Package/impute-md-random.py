...
'''
This program imputes genotypes with missing data in a VCF file using a random process.
...
def main():
    ...
    # impute genotypes with missing data in a VCF file using a random process
    impute_md_random(args.threads_num, args.input_vcf_file, args.output_vcf_file, args.tvi_list)
...
def build_parser():
    ...
    description = 'Description: This program imputes genotypes with missing data in a VCF file using a random process.'
...
def impute_md_random(threads_num, input_vcf_file, output_vcf_file, tvi_list):
    '''
    Impute genotypes with missing data in a VCF file using a random process.
    '''
...
def process_variant(thread_id, tvi_list, sample_number, data_dict, result_list):
    '''
    Process a variant and impute its genotypes with missing data using a random process if necessary.
    '''

    accepted_gt_list = [('0', '0'), ('0', '1'), ('1', '1')]
    ...
    # impute missing data with a random value
    for i in range(sample_number):
        if sample_gt_left_list[i] == '.' or sample_gt_right_list[i] == '.':
            random_gt_number = random.randint(0, len(accepted_gt_list) - 1)
            sample_gt_list[i] = f'{accepted_gt_list[random_gt_number][0]}{sample_sep_list[i]}{accepted_gt_list[random_gt_number][1]}'
