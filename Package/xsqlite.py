...
def create_vcf_samples_alleles_index(conn):
    '''
    Create the unique index "vcf_samples_alleles_index" with thes column "variant_id", "allele_id" and "sample_id" on the table "vcf_samples_alleles"
    '''
...
def query_imputed_alleles(conn, imputed_md_id):
    '''
    Get a dictionary corresponding to rows of alleles with imputations.
    '''
...
def query_species_allele_frequencies(conn, md_symbol):
    '''
    Get a dictionary corresponding to rows of individual allele frequencies per species of variant (alleles with missing data are not considered).
    '''
