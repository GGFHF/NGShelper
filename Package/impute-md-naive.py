...
def main():
    ...
    impute_md_naive(args.threads_num, args.input_vcf_file, args.output_vcf_file, args.tvi_list)
...
def impute_md_naive(threads_num, input_vcf_file, output_vcf_file, tvi_list):
    '''
    Impute genotypes with missing data in a VCF file using a naive process.
    '''
    ...
    if most_frequent_gt != '':
        for i in range(sample_number):
            if sample_gt_left_list[i] == '.' or sample_gt_right_list[i] == '.':
                sample_gt_list[i] = f'{most_frequent_gt[0]}{sample_sep_list[i]}{most_frequent_gt[2]}'
