...
'''
This program converts a VCF file to files (.dat & .ped) used by MACH.
...
def main():
    ...
    # convert the VCF file to files (.dat & .ped) used by MACH
    convert_vcf_to_mach(args.vcf_file, args.dat_file, args.ped_file, args.md_characters, args.tvi_list)
...
def build_parser():
    ...
    description = 'Description: This program converts a VCF file to files (.dat & .ped) used by MACH.'
    ...
    parser.add_argument('--dat', dest='dat_file', help='Path of the output file .dat (mandatory).')
    parser.add_argument('--ped', dest='ped_file', help='Path of the output file .ped (mandatory).')
...
def check_args(args):
    ...
    if args.dat_file is None:
        xlib.Message.print('error', '*** The output file .dat is not indicated in the input arguments.')
        OK = False

    # check "ped_file"
    if args.ped_file is None:
        xlib.Message.print('error', '*** The output file .ped is not indicated in the input arguments.')
        OK = False
...
def convert_vcf_to_mach(vcf_file, dat_file, ped_file, md_characters, tvi_list):
    '''
    Converts a VCF file to files (.dat & .ped) used by MACH.
    '''
    ...
    while record != '':
        ...
        while record != '' and not record.startswith('##') and not record.startswith('#CHROM'):
            ...
            variant_id = f'{data_dict["chrom"]}-{data_dict["pos"]:0>8}'
    ...
    # open the output file (.dat)
    if dat_file.endswith('.gz'):
        try:
            dat_file_id = gzip.open(dat_file, mode='wt', encoding='iso-8859-1', newline='\n')
        except Exception as e:
            raise xlib.ProgramException(e, 'F004', dat_file)
    else:
        try:
            dat_file_id = open(dat_file, mode='w', encoding='iso-8859-1', newline='\n')
        except Exception as e:
            raise xlib.ProgramException(e, 'F003', dat_file)

    # write variant records
    for variant_id in variant_id_list:
        dat_file_id.write(f'M {variant_id}\n')

    # close file .dat
    dat_file_id.close()

    # print OK message
    xlib.Message.print('info', f'The converted file {os.path.basename(dat_file)} is created.')

    # open the output file (.ped)
    if ped_file.endswith('.gz'):
        try:
            ped_file_id = gzip.open(ped_file, mode='wt', encoding='iso-8859-1', newline='\n')
        except Exception as e:
            raise xlib.ProgramException(e, 'F004', ped_file)
    else:
        try:
            ped_file_id = open(ped_file, mode='w', encoding='iso-8859-1', newline='\n')
        except Exception as e:
            raise xlib.ProgramException(e, 'F003', ped_file)
    ...
    for i in range(sample_number):
        ...
        ped_file_id.write(f'{sample_list[i]}\t{sample_list[i]}\t0\t0\t2\t{sample_variant_gt_left_list_text}\n')

    # close file .ped
    ped_file_id.close()

    # print OK message
    xlib.Message.print('info', f'The converted file {os.path.basename(ped_file)} is created.')
